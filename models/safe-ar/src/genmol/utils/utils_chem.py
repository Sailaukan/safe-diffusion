...
MIX_SEQUENCES_TIMEOUT_SECONDS = 60
...
def _mix_sequences_worker(queue, prefix_sequences, suffix_sequences, prefix, suffix, num_samples):
    """Worker run in a subprocess to isolate RDKit native crashes (SIGSEGV) in MolSlicer."""
    try:
...
def mix_sequences(prefix_sequences, suffix_sequences, prefix, suffix, num_samples=1):
    queue = Queue()
    process = Process(
        target=_mix_sequences_worker,
        args=(queue, prefix_sequences, suffix_sequences, prefix, suffix, num_samples),
    )
    process.start()
    process.join(timeout=MIX_SEQUENCES_TIMEOUT_SECONDS)
