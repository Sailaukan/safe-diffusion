...
SWEEP_PATH="${SWEEP_PATH:-sajlaukansahnazar-mbzuai/safe-mdlm-sweeps/v2ujglhn}"
RUN_PATH="${RUN_PATH:-sajlaukansahnazar-mbzuai/safe-mdlm-sweeps/${RUN_ID}}"
...
while pgrep -af "scripts/train.py" | grep -q "wandb.project=safe-mdlm-sweeps"; do
  pgrep -af "scripts/train.py" | grep "wandb.project=safe-mdlm-sweeps" || true
  sleep 30
done
