...
from genmol.backbone import SafeBertForMaskedLM
from genmol.diffusion import LogLinearNoiseSchedule, MaskedDiscreteDiffusion
...
class SafeMDLM(L.LightningModule):
    """Lightning module for SAFE masked discrete diffusion."""
    ...
    def _build_backbone(self):
        if self.config.model.get('time_conditioning', False):
            raise ValueError('SAFE-MDLM expects model.time_conditioning=False')
        return SafeBertForMaskedLM(BertConfig.from_dict(dict(self.config.model)))
    ...
    def _build_diffusion(self):
        ...
        if diffusion_config.get('engine', 'mdlm') != 'mdlm':
            raise ValueError('SAFE-MDLM expects diffusion.engine=mdlm')
        if diffusion_config.get('type', 'absorbing_state') != 'absorbing_state':
            raise ValueError('SAFE-MDLM expects diffusion.type=absorbing_state')
        if diffusion_config.get('parameterization', 'subs') != 'subs':
            raise ValueError('SAFE-MDLM expects diffusion.parameterization=subs')
        if diffusion_config.get('zero_recon_loss', False):
            raise ValueError('SAFE-MDLM expects diffusion.zero_recon_loss=False')

        noise_eps = self.config.noise.get('eps', self.config.training.sampling_eps)
        noise_schedule = LogLinearNoiseSchedule(eps=noise_eps)
        return MaskedDiscreteDiffusion(
    ...
    def training_step(self, batch, batch_idx):
        ...
        # forward process to add absorbing-state mask noise
...
# Backward-compatible aliases for older imports/checkpoints.
GenMol = SafeMDLM
